...
class PasswordUI {
    ...
    attachEvents() {
        const { slider, valueDisplay, checkboxes, refreshBtn, copyBtn } = this.elements;
        slider.addEventListener("input", () => {
            valueDisplay.innerText = slider.value;
            this.updatePassword();
        });
        checkboxes.forEach((cb) => cb.addEventListener("change", () => this.updatePassword()));
    ...
    updatePassword() {
        const { slider, upcCheck, lwcCheck, numbersCheck, symbolsCheck, passwordDisplay, entropyField, crackTimeField, strengthText, strengthLight, } = this.elements;
        const activeSets = [];
        if (lwcCheck.checked)
            activeSets.push(new LowerCaseSet());
        if (upcCheck.checked)
            activeSets.push(new UpperCaseSet());
        if (numbersCheck.checked)
            activeSets.push(new NumberSet());
        if (symbolsCheck.checked)
            activeSets.push(new SymbolSet());
        const pwd = new PasswordGenerator(activeSets).generate(Number(slider.value));
        ...
        crackTimeField.innerText = CrackTimeFormatter.format(stats.crackTimeSeconds);
...
new PasswordUI(new PasswordGenerator([]), new PasswordStatsCalculator(new SymbolSet().getAll().length), new PasswordStrength(colorCode), elements);
